(function() {
  ...
  var Storage = {
    ...
    set: function(name, val, expries) {
      ...
      localStorage.setItem(
        name,
        JSON.stringify({ value: val, expries: expriesTime })
      );
    },
  ...
  function getNumFromId(id) {
    // id menu99_001.jpg
    return parseInt(id.split('.')[0].split('_')[2]);
  }
              ...
              if (dataSorted_mini.length === 0) {
                ...
                for (var j = 0; j < dataSorted_mini.length; j++) {
                  ...
                  if (curNum > num) {
                    ...
                    // 插入
                    dataSorted_mini.splice(j, 0, data.data[i]);
                    break;
                  }
                }
              }
            ...
            if (ele.picId.substr(0, 4) === 'mini') {
              var html =
                '<div class="content-img_box">\
                                            <img data-picId="' +
                ele.picId +
                '" class="content-img" src="../static/img/' +
                menuId +
                '/' +
                ele.picId +
                '" alt="">\
              ...
              if (
                ...
                if (i === 0) {
                  ...
                  zoomSrc.splice(
                    -1,
                    1,
                    _dataSorted_mini[_dataSorted_mini.length - 1].picId
                  );
                  $zoomPic.attr('src', zoomSrc.join('/').replace('mini_', ''));
                } else {
                  var zoomSrc = $zoomPic.attr('src').split('/');
                  zoomSrc.splice(-1, 1, _dataSorted_mini[i - 1].picId);
                  $zoomPic.attr('src', zoomSrc.join('/').replace('mini_', ''));
                }
                break;
              }
    ...
    $uploadBox.off('click').click(function() {
      ...
      $fileupload
        ...
        .fileupload({
          ...
          add: function(e, data) {
            var allowTypes = ['jpg'];
            var _type = $(data.files[0].name.split('.'))
              .last()[0]
              .toLowerCase(); // 文件后缀
            var $menu = $('#menu');
            ...
            if ($.inArray(_type, allowTypes) != -1) {
              ...
              var html =
                '<div class="content-img_box">\
                                        <img data-picId="mini_' +
                newPicId +
                '" class="content-img" src="" alt="" height="133" width="133">\
            ...
            $html
              .removeAttr('id')
              .removeAttr('data-curMenuId')
              .removeAttr('data-newPicId');
            console.log('done');
          },
          progress: function(e, data) {
            ...
            $html.find('.progress-bar').css('width', progress + '%');
          }
        })
